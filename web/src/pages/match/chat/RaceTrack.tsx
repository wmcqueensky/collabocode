...
export const RaceTrack = ({ participants, isMobile = false }: any) => {
  const [animatedParticipants, setAnimatedParticipants] =
    useState(participants);

  // Simulate random progress updates
  useEffect(() => {
    const interval = setInterval(() => {
      setAnimatedParticipants((prev: any) =>
        prev.map((participant: any) => {
          if (participant.progress >= 100) return participant;
          const shouldUpdate = Math.random() < 0.3; // 30% chance to update
          if (!shouldUpdate) return participant;
          const progressIncrement = Math.random() * 5; // Random increment up to 5%
          ...
          const statuses = ['idle', 'typing', 'thinking'];
          const newStatus =
            newProgress >= 100
              ? 'complete'
              : Math.random() < 0.3
  ...
  const sortedParticipants = [...animatedParticipants].sort(
      ...
      <div className="flex-1 overflow-auto p-2 sm:p-4">
        <div className={`space-y-${isMobile ? '2' : '4'}`}>
          {sortedParticipants.map((participant, index) => {
            const isComplete = participant.progress >= 100;
