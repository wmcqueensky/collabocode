	...
	return (
		...
		>
			<div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
				...
				<button
					className="bg-sky-100 hover:bg-sky-200 text-sky-700 px-3 py-1 rounded text-xs sm:text-sm flex items-center justify-center space-x-1 w-full sm:w-auto"
					onClick={() => runTest("all")}
				>
					...
					>
						<div className="flex items-center justify-between px-2 sm:px-3 py-2 bg-gray-50 border-b border-gray-200">
							...
							<div className="flex items-center space-x-2">
								<button
									className="text-xs bg-sky-100 hover:bg-sky-200 text-sky-700 px-2 py-0.5 rounded whitespace-nowrap"
									onClick={() => runTest(index)}
								>
									Run
								</button>
								{test.status === "pass" && (
									<Check size={isMobile ? 14 : 16} className="text-sky-600" />
								)}
								{test.status === "fail" && (
