...
const InvitePlayersStep = ({
	...
	const togglePlayerSelection = (player: any) => {
		if (selectedPlayers.some((p: any) => p.id === player.id)) {
			setSelectedPlayers(
				selectedPlayers.filter((p: any) => p.id !== player.id)
			);
		} else {
					...
					{filteredPlayers.map((player: any) => (
						<div
							...
							className={`p-3 rounded-lg border transition-all cursor-pointer ${
								selectedPlayers.some((p: any) => p.id === player.id)
							...
							<div className="mt-2 flex items-center justify-between text-xs text-gray-400">
								<div>
									...
									<span className="inline-block">
										🏆 {player.problemsSolved || 0} Problems
									</span>
								</div>
